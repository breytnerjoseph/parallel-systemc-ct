ct_de = load("ct_de_10ms.dat");
tdf = load("tdf_10ms.dat") 
...
hold on;
grid on;
plot(ct_de(:,1), ct_de(:,2) , '-r', 'LineWidth', 1);
plot(tdf(:,1), tdf(:,2) , '-k', 'LineWidth', 1);
plot(speed_matlab(:,1), speed_matlab(:,2) , '--b', 'LineWidth', 1);
...
figure(2);
title('Gear shifting - SystemC datapoints spaced 10 ms');
hold on;
grid on;
plot(ct_de(:,1), ct_de(:,3) , '-r', 'LineWidth', 1);
plot(tdf(:,1), tdf(:,3) , '-k', 'LineWidth', 1);
plot(gear_matlab(:,1), gear_matlab(:,2) , '--b', 'LineWidth', 1);
